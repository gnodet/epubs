...
AUTHOR="Vatican"
ABBR="dse" 
UUID="3de381d5-a71f-4bdf-962a-2bc36abab4d9"
OUTPUT="Doctrine-Sociale-Eglise.epub"     
...
cd org
if [ ! -e rc_pc_justpeace_doc_20060526_compendio-dott-soc_fr.html ]
then
  wget http://www.vatican.va/roman_curia/pontifical_councils/justpeace/documents/rc_pc_justpeace_doc_20060526_compendio-dott-soc_fr.html
fi
...
do           
  ...
  perl -e ' 
      sub translate {
        my ($str1, $str2, $str3) = @_;
        $str2 =~ s/ |,|\xab|\xbb|\)|://g;
        $str2 =~ s/\xe9|\xe8|\xea/e/g;
        $str2 =~ s/\xe2|\xe0/a/g;
        $str2 =~ s/\xf4/o/g;
        $str2 =~ s/\xef/i/g;
        $str2 =~ s/\xc9|\xc8|\xca/E/g;
        $str2 =~ s/\xc0/A/g;
	  ...
	  $text =~ s/\x92/\&rsquo;/g;
	  $text =~ s/\x96/\&ndash;/g;
	  $text =~ s/(<a name=")([^"]*)(">)/&translate($1,$2,$3)/ge;
	  $text =~ s/(<a href="#)([^"]*)(">)/&translate($1,$2,$3)/ge;
  ...
  perl -e '
      ...
      $file = $ARGV[0];  local( *FH ) ;  open( FH, $file ); my $text = do { local( $/ ) ; <FH> } ; 
      # Fix missing accents
	  $text =~ s/\bEvangile\b/\&Eacute;vangile/g;
	  $text =~ s/\bEglise\b/\&Eacute;glise/g;
	  $text =~ s/\bEcriture\b/\&Eacute;criture/g;
	  $text =~ s/\bEgypte\b/\&Eacute;gypte/g;
	  $text =~ s/\bEpoux\b/\&Eacute;poux/g;
	  $text =~ s/\bEternel\b/\&Eacute;ternel/g;
	  $text =~ s/\bEv\&ecirc;que\b/\&Eacute;v\&ecirc;que/g;
	  $text =~ s/\bElisabeth\b/\&Eacute;lisabeth/g;
	  $text =~ s/\bEGLISE\b/\&Eacute;GLISE/g;
	  $text =~ s/\bMERE\b/M\&Egrave;RE/g;
	  $text =~ s/\bPERE\b/P\&Egrave;RE/g;
	  $text =~ s/\bMEDIATION\b/M\&Eacute;DIATION/g;
	  $text =~ s/\bMISERICORDE\b/MIS\&Eacute;RICORDE/g;
	  $text =~ s/\bHERITAGE\b/H\&Eacute;RITAGE/g;     
	  $text =~ s/\bMYSTERE\b/MYST\&Egrave;RE/g;        
	  $text =~ s/\bVERITE\b/V\&Eacute;RIT\&Eacute;/g;
      # Fix typos
	  ...
	  $text =~ s/\. A /. \&Agrave; /g;
	  $text =~ s/L '"'"'/L'"'"'/g; 
	  $text =~ s/Lanecessitedunegrande%C5%93uvreeducativeetculturelle/Lanecessitedunegrandeoeuvreeducativeetculturelle/g;
	  # Fix bad html
  ...
  perl -e '                                                                                
	  ...
	  $text =~ s/%C3%89/E/g;
      # Foot notes	                                        
	  $text =~ s/<\/head>/<style type="text\/css"> .footnote { font-family: Verdana; font-size: 80%; font-weight: bold }<\/style><\/head>/;
	  $text =~ s/(?<!<p>)<sup>([1-9][0-9]*)<\/sup>/<sup class="footnote"><a href="#n$1" id="rn$1">$1<\/a><\/sup>/g;
	  $text =~ s/<p><sup>([1-9][0-9]*)<\/sup>/<p><sup class="footnote"><a href="#rn$1" id="n$1">$1<\/a><\/sup> /g;
	  $text =~ s/\. (<sup.*?<\/sup>)/\&nbsp;$1./g;
	  print $text;' $FILE > $FILE.tmp ; cp $FILE.tmp $FILE ; rm $FILE.tmp
...
<svg xmlns="http://www.w3.org/2000/svg" 
	...
	<text style="font-size:72px;font-style:normal;font-weight:bold;fill:#000000;font-family:Brush Script MT" 
		  x="265" y="240">
		<tspan style="text-align:center;text-anchor:middle">Doctrine Sociale</tspan>
	</text>
	...
	<text style="font-size:64px;font-style:normal;font-weight:bold;fill:#000000;font-family:Brush Script MT" 
		  x="265" y="374">
        <tspan style="text-align:center;text-anchor:middle">l'Église</tspan>
    </text>
</svg>
EOF
...
echo "    Generating toc"
xsltproc ../../../dse-toc.xsl ../../../dse-toc.xml > toc.ncx
...
EOF
/Applications/calibre.app/Contents/MacOS/ebook-convert $OUTPUT ../../$OUTPUT --extra-css overrides.css
