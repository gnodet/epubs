...
ABBR="p-vi" 
FULL="paul_vi"
UUID="3de381d5-a71f-4bdf-962a-2bc36abab4d7"
OUTPUT="Encycliques-Paul-VI.epub"     
...
do           
  ...
  perl -e '
      ...
      # Fix <br>                   
	  $text =~ s/<br>\s*20. Vers une condition plus humaine<br>\s*<br>/<br><i>Vers une condition plus humaine<\/i><br><br>20. /sg;
      $text =~ s/<br>\s*<\/i><br>/<\i><br><br>/sgi;
      $text =~ s/<br>\s*<\/i>\s*<\/font>\s*<br>/<\i><\/font><br><br>/sgi;
      $text =~ s/(<\/p>\s*)(<p[^>]*>)(<br>)/$3$1$2/sgi;
      while (($text =~ s/(<p align="center">)(([^<]*|<[a-qr-z])*)<br>/$1$2<br\/>/sg) > 0) { };
      $text =~ s/<br>\s*<br>/<\/p><p>/gsm;
  ...
  perl -e '
	  ...
	  $text =~ s/PREMIERE/PREMI\&Egrave;RE/g;
	  $text =~ s/DEUXIEME/DEUXI\&Egrave;ME/g;  
	  $text =~ s/ACTION A ENTREPRENDRE/ACTION \&Agrave; ENTREPRENDRE/g;
      # Fix typos
	  ...
	  $text =~ s/\. A /. \&Agrave; /g;
	  $text =~ s/L '"'"'/L'"'"'/g; 
	  $text =~ s/\]'"'"'Esprit/l'"'"'Esprit/g;
	  $text =~ s/\.\.\. 1\. Les maris/\.\.\. Les maris/g;
	  $text =~ s/\)([a-z])/\) \1/g;    
	  $text =~ s/!'"'"'homme/l'"'"'homme/g;
	  $text =~ s/1\. (LES)/I. $1/g;
	  $text =~ s/2\. (L'"'"'.GLISE)/II. $1/g;
	  $text =~ s/3\. (L'"'"'ACTION)/III. $1/g;
	  $text =~ s/1\. (L'"'"'ASSISTANCE)/I. $1/g;
	  $text =~ s/2\. (L'"'"'.QUIT)/II. $1/g;
	  $text =~ s/3\. (LA)/III. $1/g;     
...
#	  $text =~ s/(<p>[0-9][0-9]*) - /$1. /g;
	  $text =~ s/(<p>|<p [^>]*>|<p><span>|<p><span [^>]*>|<p [^>]*><span [^>]*>)([0-9][0-9]*)(\. | - | \&ndash; )/$1<a class="numpara" id="$2.">$2.<\/a> /g;
