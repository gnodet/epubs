...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        indexHtml.append("<h3>Lettres apostoliques</h3>");
        indexHtml.append("<table>");

        Document enclist = dbf.newDocumentBuilder().parse(Main.class.getResourceAsStream("let-list.xml"));
...
//        int i = 24; {
            ...
            String output = "let_" + date + "_hf_" + full + "_" + title.toLowerCase().replaceAll("\\s", "-").replaceAll("æ", "ae");
...
//            if (!title.contains("Evangelii Gaudium")) continue;
            ...
            // Avoid redirects
            String base = "https://w2.vatican.va/content/john-paul-ii/fr/apost_letters/1984/documents/";
        ...
        template = template.replace("${TITLE}", "Lettres apostoliques");
        template = template.replace("${CONTENT}", indexHtml.toString());
        writeToFile(template, "target/site/lettres.html");
    }
    ...
    private static String cleanMain(String document) {
        ...
        document = document.replaceAll("SÅ“urs", "Sœurs");
        document = document.replaceAll("Soeurs", "Sœurs");
...
//        document = document.replaceAll("<li>(.*?)</li>", "<div class=\"bullet\">$1</div>");
        ...
        document = document.replaceAll("<p></div></div> <br /> <hr /></p>", "");
